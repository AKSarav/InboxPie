...
(function () {
  ...
  async function init() {
    ...
    var headerCollapseBtn = document.getElementById("headerCollapseBtn");
    var headerRevealBtn   = document.getElementById("headerRevealBtn");
    ...
    browser.runtime.onMessage.addListener((msg) => {
      ...
      // Enrichment indexing events (ADR 001) — share state with intelligence.js via window._ip
      if (msg.action === "enrichmentStarted") {
        window._ip = window._ip || {};
        window._ip.enrichStats = { status: 'indexing', total: msg.total };
        if (window._ip.onEnrichUpdate) window._ip.onEnrichUpdate(window._ip.enrichStats);
      }
      if (msg.action === "enrichmentDone") {
        window._ip = window._ip || {};
        window._ip.enrichStats = {
          status: 'done',
          senders: msg.senders,
          domains: msg.domains,
          edges: msg.edges,
          indexedMessages: msg.indexedMessages,
          totalMessages: msg.totalMessages,
          skipped: msg.totalMessages - msg.indexedMessages,
          indexedAt: new Date().toISOString(),
        };
        if (window._ip.onEnrichUpdate) window._ip.onEnrichUpdate(window._ip.enrichStats);
      }
      // Knowledge extraction progress (ADR 003 Phase 2)
      if (msg.action === "extractionProgress" || msg.action === "extractionComplete" || msg.action === "extractionLog") {
        window._ip = window._ip || {};
        if (window._ip.onExtractionProgress) window._ip.onExtractionProgress(msg);
      }
      // FalkorDB Lite install progress
      if (msg.action === "falkorInstallProgress") {
        window._ip = window._ip || {};
        if (window._ip.onFalkorInstallProgress) window._ip.onFalkorInstallProgress(msg.line);
      }
      // Graph indexing progress (new inboxpie-db indexer)
      if (msg.action === "indexingProgress" || msg.action === "indexingComplete") {
        window._ip = window._ip || {};
        if (window._ip.onVectorIndexProgress) window._ip.onVectorIndexProgress(msg);
      }
    });
  }
  ...
  function renderScanFolderDropdown() {
    ...
    scanFolderList.forEach((folder) => {
      ...
      // Show index-status badge for previously indexed folders
      if (folder.isIndexedBody) {
        const badge = document.createElement("span");
        badge.className = "folder-indexed-badge folder-indexed-body";
        badge.title = `Body indexed (${(folder.indexedBodyCount || 0).toLocaleString()} mails)`;
        badge.textContent = "✓ body";
        label.appendChild(badge);
      } else if (folder.isIndexedMeta) {
        const badge = document.createElement("span");
        badge.className = "folder-indexed-badge folder-indexed-meta";
        badge.title = `Metadata indexed (${(folder.indexedMetaCount || 0).toLocaleString()} mails)`;
        badge.textContent = "✓ meta";
        label.appendChild(badge);
      } else if (folder.isScanned) {
        const badge = document.createElement("span");
        badge.className = "folder-indexed-badge folder-scanned";
        badge.title = "Scanned — not yet indexed for AI";
  ...
  // ══════════════════════════════════════════
  //  INDEX MODE MODAL (one-time preference)
  // ══════════════════════════════════════════
    ...
    try {
      ...
      window._ip.openSenderReview = function (email) {
        ...
        pool.filter(function (m) { return m.senderEmail === email; })
            .forEach(function (m) { selectedIds.add(m.id); });
        updateStats();
        if (selectedIds.size > 0) showSelectionReviewModal();
      };
      ...
      // Trigger background enrichment DB build (ADR 001)
      browser.runtime.sendMessage({ action: "enrichMessages", messages: allMessages })
        .catch(function () { /* enrichment failure is non-fatal */ });

      // Auto-build vector index in background. Each folder is indexed in its own
      // read_mode (folder-level, default metadata; change per folder in Settings).
      if (allMessages.length > 0) {
        window._ip.indexing = true;
        browser.runtime.sendMessage({
          action:   "buildVectorIndex",
          messages: allMessages,
        }).catch(function () { window._ip.indexing = false; });
      }
  ...
  var _headerUserCollapsed = false; // true when user manually collapsed

  function setHeaderCollapsed(collapsed) {
    var header      = document.getElementById("header");
    var revealBar   = document.getElementById("headerRevealBar");
    if (!header || !revealBar) return;
    if (collapsed) {
